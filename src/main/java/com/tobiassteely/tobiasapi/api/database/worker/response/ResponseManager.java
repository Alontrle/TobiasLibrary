package com.tobiassteely.tobiasapi.api.database.worker.response;
...
public class ResponseManager extends Worker {
    ...
    public ResponseManager() {
        ...
        this.responses = TobiasAPI.getInstance().getMongo().getResponses();
    }
    ...
    public Boolean runWorker(long start) {
        for (Document doc : responses.find(new Document("destination", TobiasAPI.getInstance().getMongo().getId()).append("loaded", false))) {
            new Thread(() -> {
    ...
    public void loadManager() {
        for (Document doc : responses.find(new Document("destination", TobiasAPI.getInstance().getMongo().getId()))) {
            for (ResponseEventHandler eventHandler : events) {
